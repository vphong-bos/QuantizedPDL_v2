...
def export_qoperator_static(
    ...
    model_for_quant = maybe_prepare_model_for_rewrite(
        input_model_path=input_model_path,
        fix_input_shape_flag=fix_input_shape_flag,
        fixed_batch=fixed_batch,
        image_height=image_height,
    ...
    tmp_input_model = input_model_path
    if fix_input_shape_flag:
        tmp_input_model = os.path.join(
            os.path.dirname(output_model_path),
            "_tmp_qoperator_fixed_input.onnx",
        )
        save_model(model_for_quant, tmp_input_model)
...
def create_optimized_variants(
    ...
    num_workers: int,
    calib_samples: int,
    qoperator_activation_type: str,
    qoperator_weight_type: str,
    qoperator_calibration_method: str,
    debug_batch: bool,
) -> Dict[str, str]:
    ...
    for variant in variants:
        ...
        try:
            if variant == "manual_hand_opt":
            ...
            elif variant == "qoperator_static":
                if not fp32_model_path:
                    raise ValueError("--fp32_model is required for qoperator_static")
                ...
                export_qoperator_static(
                    ...
                    activation_type=qoperator_activation_type,
                    weight_type=qoperator_weight_type,
                    calibration_method=qoperator_calibration_method,
                    fix_input_shape_flag=fix_input_shape_flag,
                    fixed_batch=fixed_batch,
                    debug_batch=debug_batch,
                )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="FP32 ONNX model path. Required for qoperator_static.",
    )

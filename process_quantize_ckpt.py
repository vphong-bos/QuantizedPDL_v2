import argparse
from collections import defaultdict
from pathlib import Path

import onnx
import torch
from onnx import numpy_helper


WEIGHT_Q_SUFFIX = ".weight_q"
BIAS_Q_SUFFIX = ".bias_q"


def to_torch_tensor(initializer):
    return torch.from_numpy(numpy_helper.to_array(initializer).copy())


def build_initializer_map(onnx_model):
    return {initializer.name: initializer for initializer in onnx_model.graph.initializer}


def build_graph_maps(onnx_model):
    producer = {}
    consumers = defaultdict(list)

    for node in onnx_model.graph.node:
        for output_name in node.output:
            producer[output_name] = node
        for input_name in node.input:
            consumers[input_name].append(node)

    return producer, consumers


def normalize_prefix(prefix):
    if prefix == "patch_embed":
        return "model.patch_embeddings"
    return f"model.{prefix}"


def find_compute_node_from_weight_qdq(weight_tensor_name, consumers):
    queue = [weight_tensor_name]
    seen_tensors = set()
    passthrough_ops = {"Transpose", "Reshape", "Identity", "Cast", "Flatten", "Squeeze", "Unsqueeze"}

    while queue:
        tensor_name = queue.pop(0)
        if tensor_name in seen_tensors:
            continue
        seen_tensors.add(tensor_name)

        for consumer in consumers.get(tensor_name, []):
            if consumer.op_type in {"Conv", "MatMul", "Gemm"}:
                return consumer
            if consumer.op_type in passthrough_ops:
                queue.extend(consumer.output)

    return None


def find_activation_input_name(compute_node, weight_prefix):
    weight_roots = {
        f"{weight_prefix}.weight_qdq",
        f"{weight_prefix}.weight_q",
        f"{weight_prefix}.weight",
    }

    for input_name in compute_node.input:
        if any(input_name.startswith(root) for root in weight_roots):
            continue
        if f"{weight_prefix}.weight" in input_name:
            continue
        return input_name

    if compute_node.op_type == "Conv":
        return compute_node.input[0]
    if len(compute_node.input) >= 2:
        return compute_node.input[0]
    return None


def extract_qparams_from_dq_tensor(tensor_name, producer, initializers):
    dq_node = producer.get(tensor_name)
    if dq_node is None or dq_node.op_type != "DequantizeLinear" or len(dq_node.input) < 3:
        return None

    scale_name = dq_node.input[1]
    zero_point_name = dq_node.input[2]

    if scale_name not in initializers or zero_point_name not in initializers:
        return None

    return {
        "scale": to_torch_tensor(initializers[scale_name]),
        "zeropoint": to_torch_tensor(initializers[zero_point_name]),
    }


def find_output_quant_params(prefix, compute_node, consumers, producer, initializers):
    output_tensor_name = compute_node.output[0]

    if compute_node.op_type in {"MatMul", "Gemm"}:
        for consumer in consumers.get(output_tensor_name, []):
            if consumer.op_type != "Add":
                continue
            if any(input_name == f"{prefix}.bias_qdq" for input_name in consumer.input):
                output_tensor_name = consumer.output[0]
                break

    for consumer in consumers.get(output_tensor_name, []):
        if consumer.op_type != "QuantizeLinear" or len(consumer.input) < 3:
            continue

        scale_name = consumer.input[1]
        zero_point_name = consumer.input[2]
        if scale_name in initializers and zero_point_name in initializers:
            return {
                "scale": to_torch_tensor(initializers[scale_name]),
                "zeropoint": to_torch_tensor(initializers[zero_point_name]),
            }

    return None


def collect_quantized_layer_state(onnx_model):
    initializers = build_initializer_map(onnx_model)
    producer, consumers = build_graph_maps(onnx_model)

    state_dict = {}
    missing_input_qparams = []
    missing_output_qparams = []

    quantized_prefixes = sorted(
        initializer.name[: -len(WEIGHT_Q_SUFFIX)]
        for initializer in onnx_model.graph.initializer
        if initializer.name.endswith(WEIGHT_Q_SUFFIX)
    )

    for prefix in quantized_prefixes:
        export_prefix = normalize_prefix(prefix)

        weight_name = f"{prefix}.weight_q"
        weight_scale_name = f"{prefix}.weight_scale"
        weight_zero_point_name = f"{prefix}.weight_zero_point"

        state_dict[f"{export_prefix}.weight"] = to_torch_tensor(initializers[weight_name])
        state_dict[f"{export_prefix}.weight_scale"] = to_torch_tensor(initializers[weight_scale_name])
        state_dict[f"{export_prefix}.weight_zeropoint"] = to_torch_tensor(initializers[weight_zero_point_name])

        bias_name = f"{prefix}.bias_q"
        bias_scale_name = f"{prefix}.bias_scale"
        bias_zero_point_name = f"{prefix}.bias_zero_point"

        if bias_name in initializers:
            state_dict[f"{export_prefix}.bias"] = to_torch_tensor(initializers[bias_name])
        if bias_scale_name in initializers:
            state_dict[f"{export_prefix}.bias_scale"] = to_torch_tensor(initializers[bias_scale_name])
        if bias_zero_point_name in initializers:
            state_dict[f"{export_prefix}.bias_zeropoint"] = to_torch_tensor(initializers[bias_zero_point_name])

        compute_node = find_compute_node_from_weight_qdq(f"{prefix}.weight_qdq", consumers)
        if compute_node is None:
            missing_input_qparams.append(prefix)
            missing_output_qparams.append(prefix)
            continue

        activation_input_name = find_activation_input_name(compute_node, prefix)
        input_qparams = None
        if activation_input_name is not None:
            input_qparams = extract_qparams_from_dq_tensor(activation_input_name, producer, initializers)

        if input_qparams is not None:
            state_dict[f"{export_prefix}.input_scale"] = input_qparams["scale"]
            state_dict[f"{export_prefix}.input_zeropoint"] = input_qparams["zeropoint"]
        else:
            missing_input_qparams.append(prefix)

        output_qparams = find_output_quant_params(prefix, compute_node, consumers, producer, initializers)
        if output_qparams is not None:
            state_dict[f"{export_prefix}.output_scale"] = output_qparams["scale"]
            state_dict[f"{export_prefix}.output_zeropoint"] = output_qparams["zeropoint"]
        else:
            missing_output_qparams.append(prefix)

    return state_dict, missing_input_qparams, missing_output_qparams


def extract_quantize_ckpt(ckpt_path, output_path):
    onnx_model = onnx.load(ckpt_path)
    state_dict, missing_input_qparams, missing_output_qparams = collect_quantized_layer_state(onnx_model)

    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(state_dict, output_path)

    print(f"Saved {len(state_dict)} tensors to {output_path}")
    print(f"Layers without explicit input activation qparams: {len(missing_input_qparams)}")
    for prefix in missing_input_qparams:
        print(f"  - {prefix}")
    print(f"Layers without explicit output activation qparams: {len(missing_output_qparams)}")
    for prefix in missing_output_qparams:
        print(f"  - {prefix}")

    return state_dict


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract quantized tensors and activation qparams from a QDQ ONNX model.")
    parser.add_argument("ckpt_path", help="Path to the quantized ONNX model")
    parser.add_argument("output_path", help="Path to save the extracted torch state_dict")
    args = parser.parse_args()

    extract_quantize_ckpt(args.ckpt_path, args.output_path)
    breakpoint()